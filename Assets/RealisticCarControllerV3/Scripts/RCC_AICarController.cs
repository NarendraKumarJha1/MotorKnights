...
public class RCC_AICarController : MonoBehaviour {
	...
	// Waypoint Container.
	private WaypointCircuit waypointsContainer;
    ...
    {		
        ...
        if (_AIType == AIType.FollowWaypoints)
        {
            ...
            {
                navigatorInput = Mathf.Clamp(nextWaypointPosition.normalized.x * 1.5f, -1f, 1f);
            }
        }
        else
        {
    ...
    void OnTriggerEnter (Collider col){
		...
		/*if(col.gameObject.GetComponent<RCC_AIBrakeZone>() || col.gameObject.CompareTag("BreakZone")){
            ...
            try
            {
                VehicleNitro.instance.AI_ActivateNitro();
            }
            catch (Exception e)
            {
    ...
    private void RayCasting()
    {
        ...
        RaycastHit hit;
        Debug.DrawRay(transform.position + transform.forward + transform.right, Quaternion.AngleAxis(10, transform.up) * transform.forward * wideRayLength, Color.white);
        Debug.DrawRay(transform.position + transform.forward - transform.right, Quaternion.AngleAxis(-10, transform.up) * transform.forward * wideRayLength, Color.white);
        Debug.DrawRay(transform.position + transform.forward, transform.forward * wideRayLength, Color.white);
        ...
        {
            if (hit.collider.tag == "Hurdle" && !isRight && !isMiddle)
            {
        ...
        if (Physics.Raycast(transform.position + transform.forward - transform.right, Quaternion.AngleAxis(-10, transform.up) * transform.forward, out hit, wideRayLength))
        {
            if (hit.collider.tag == "Hurdle" && !isLeft && !isMiddle)
            {
                Debug.DrawRay(transform.position + transform.forward - transform.right, Quaternion.AngleAxis(-10, transform.up) * transform.forward * wideRayLength, Color.red);
                isRight = true;
                rayInput = Mathf.Lerp(2, 0, hit.distance / wideRayLength);
        ...
        {
            if (hit.collider.tag == "Hurdle" && !isLeft && !isRight)
            {
