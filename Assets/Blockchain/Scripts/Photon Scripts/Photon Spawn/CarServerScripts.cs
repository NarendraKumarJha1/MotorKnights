    ...
    private void OnPowerUsedCallback(bool active, int actorNo, PowerUpsHandler.PowerUpType pwType)
    {
        ...
        switch (pwType)
        {
            case PowerUpsHandler.PowerUpType.Boost:
                ...
                break;
            case PowerUpsHandler.PowerUpType.Conversion:
                this.GetComponent<PowerUpsHandler>().StartConversion(actorNo);
                break;
            case PowerUpsHandler.PowerUpType.Missile:
                Debug.LogError("Missile to be handled");
                this.GetComponent<PowerUpsHandler>().MissileUsed();
                break;
        }
    }
